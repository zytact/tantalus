...
Refresh invokes the `refreshUsage` command for every enabled provider. The button becomes disabled, reads `Refreshing`, and sets `aria-busy=true`. Ctrl+R and Cmd+R use the same action, including from Settings, though the busy button is then out of view. Refresh now in the tray calls the same main-process path.
