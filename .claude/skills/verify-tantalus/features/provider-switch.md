...
The main process saves the proposed choice before changing live state. Turning a provider off clears its reading, removes its allowance block and tray rows, and prevents credential reads. Turning one on refreshes it immediately. A save failure preserves the previous setting and shows an alert.
