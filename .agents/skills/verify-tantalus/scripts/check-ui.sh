...
for needle in "Short window" "Long window" "Reset credits" "Refresh" "Auto-refreshes every 5 minutes" "usage-snapshot" "cached_usage" "refresh_usage"; do
  if grep -rq "$needle" src/main.tsx src-tauri/src/lib.rs 2>/dev/null; then
