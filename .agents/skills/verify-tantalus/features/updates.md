...
A release update appears below the Allowance header, in Settings, and as `Update to v<x>` in the tray. The remote access page never shows it. Install update disables while downloading, verifies the Ed25519 signature, installs the platform bundle, and relaunches. Failures restore the action and show an alert.
